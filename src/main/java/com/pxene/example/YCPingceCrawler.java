 ...
 */
public class YCPingceCrawler {
    private static final Logger logger = LoggerFactory.getLogger(YCPingceCrawler.class);

    public static void main(String[] args) throws IOException, InterruptedException {
        PropertyConfigurator.configure(YCPingceCrawler.class.getResource("/log4j.properties"));
        ...
        PageExtract page1 = PageExtract.builder().httpSender()
                ...
                .enableAjax().end().htmlFetcher()
                .configMultiValueColumn("url", "h3[class='title'] > a:nth-child(1)", "href")
                .end().build();
        ...
        String url = "http://www.bitauto.com/pingce/?page=%d";
        String refer = "http://www.bitauto.com/pingce/";
        for (int i = 1; i <= 1; i++) {
            String realUrl = String.format(url, i);
            long rs = new PipeLine().pipe(realUrl, refer, storage, page1, page2);
